...
const CONFIG_IO_TIMEOUT_MS = 10_000;

function remainingConfigIoTimeout(deadlineAt: number): number {
  const remaining = deadlineAt - Date.now();
  if (remaining <= 0) {
    const error = new Error('Configuration I/O deadline exceeded.') as NodeJS.ErrnoException;
...
class ConfigManager {
  ...
  async init() {
    ...
    const fallbackInMemory = (firstRun: boolean) => {
      this.config = this.getDefaultConfig();
      this.config['version'] = VERSION;
      this._isFirstRun = firstRun;
      if (!firstRun) {
    ...
    try {
      await runWithAbortableTimeout(
        (_signal) => fs.mkdir(configDir, { recursive: true }),
        CONFIG_IO_TIMEOUT_MS,
        `Create configuration directory ${configDir}`,
      );
    } catch (error) {
      console.error('Failed to prepare config directory; continuing in memory:', error);
      fallbackInMemory(false);
      return;
    }

    try {
      const configData = await runWithAbortableTimeout(
        (signal) => fs.readFile(this.configPath, { encoding: 'utf8', signal }),
        CONFIG_IO_TIMEOUT_MS,
        `Read configuration ${this.configPath}`,
      );
      ...
      fallbackInMemory(missing);

      if (!missing && !malformed) {
        // Do not overwrite an existing config that is merely slow, locked, or
        // temporarily inaccessible. Startup continues from bounded in-memory
        // defaults and a later restart may recover the authoritative file.
        console.error('Failed to read config within its I/O boundary; continuing in memory:', error);
        return;
      }
      ...
      try {
        ...
        console.error('Failed to persist default configuration; continuing in memory:', saveError);
      }
    }
  }
  ...
  private async writeConfigToDisk(): Promise<void> {
    const deadlineAt = Date.now() + CONFIG_IO_TIMEOUT_MS;
    const configDir = path.dirname(this.configPath);
    const tempPath = path.join(
      configDir, `.${path.basename(this.configPath)}.${process.pid}.${randomUUID()}.tmp`,
    );
    let published = false;
    try {
      await runWithAbortableTimeout(
        (_signal) => fs.mkdir(configDir, { recursive: true }),
        remainingConfigIoTimeout(deadlineAt),
        `Create configuration directory ${configDir}`,
      );
      await runWithAbortableTimeout(
        (signal) => fs.writeFile(tempPath, JSON.stringify(this.config, null, 2), { encoding: 'utf8', signal }),
        remainingConfigIoTimeout(deadlineAt),
        `Stage configuration ${tempPath}`,
      );
      await runWithAbortableTimeout(
        (_signal) => renameReplacingWithRetry(tempPath, this.configPath, { deadlineAt }),
        remainingConfigIoTimeout(deadlineAt),
        `Publish configuration ${this.configPath}`,
      );
      published = true;
    } finally {
      if (!published) {
        try {
          await runWithAbortableTimeout(
            (_signal) => fs.rm(tempPath, { force: true }),
            1_000,
            `Clean staged configuration ${tempPath}`,
          );
        } catch {
          // A timed-out cleanup must not replace the original persistence error.
        }
      }
    }
  }
