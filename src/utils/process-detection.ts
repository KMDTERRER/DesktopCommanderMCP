...
export function analyzeProcessState(output: string, pid?: number): ProcessState {
  ...
  // Terminal/PTY backends may normalize or omit the trailing prompt space and
  // commonly leave one or more blank lines after cursor-control sequences.
  const normalizedLastLine = lastLine.trimEnd();
  const allPrompts = Object.values(REPL_PROMPTS).flat();
  const detectedPrompt = allPrompts.find(prompt =>
    normalizedLastLine.endsWith(prompt.trimEnd())
  );
