...
function loadDocxZip(buf: Buffer): DocxZipContents {
    preflightZipContainer(buf, {
        ...
        selectEntry: (name) => name.endsWith('.xml') || name.endsWith('.rels'),
        label: 'DOCX',
    });
    ...
    assertByteLengthWithin(documentXml, MAX_DOCX_XML_BYTES, 'DOCX document.xml');
...
export class DocxFileHandler implements FileHandler {
    ...
    async read(path: string, options?: ReadOptions): Promise<FileResult> {
        ...
        const buf = await readFileBounded(path, MAX_DOCX_INPUT_BYTES, signal, 'DOCX input');
        const { zip, documentXml } = loadDocxZip(buf);
        const outputLimit = normalizeOutputBudget(options?.maxOutputBytes, MAX_DOCX_OUTPUT_BYTES);
        ...
        if (wantsRaw) {
            ...
            assertByteLengthWithin(content, outputLimit, 'DOCX XML read result');
            return {
                content,
        ...
        const content = outline + headerFooterInfo +
        ...
        assertByteLengthWithin(content, outputLimit, 'DOCX outline result');
        return {
            content,
            mimeType: 'text/plain',
            metadata: { isDocx: true, lineCount: totalLines },
