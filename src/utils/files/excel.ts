 ...
 * Handles reading, writing, and editing Excel XLSX files (.xlsx)
 */
...
const FILE_SIZE_LIMIT = MAX_EXCEL_INPUT_BYTES;
...
${JSON.stringify(data)}`;
        const outputLimit = normalizeOutputBudget(options?.maxOutputBytes, MAX_EXCEL_OUTPUT_BYTES);
        assertByteLengthWithin(content, outputLimit, 'Excel read result');
    ...
    private async loadWorkbook(path: string, signal?: AbortSignal): Promise<ExcelJS.Workbook> {
        const readSignal = signal ?? new AbortController().signal;
        const buffer = await readFileBounded(path, MAX_EXCEL_INPUT_BYTES, readSignal, 'Excel XLSX input');
        ...
        if (readSignal.aborted) {
            const error = new Error('Excel XLSX read aborted') as NodeJS.ErrnoException;
