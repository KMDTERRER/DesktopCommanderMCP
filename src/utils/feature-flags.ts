...
const FEATURE_FLAGS_FETCH_TIMEOUT_MS = 3_000;
const FEATURE_FLAGS_IO_TIMEOUT_MS = 3_000;
...
class FeatureFlagManager {
  ...
  private async loadFromCache(): Promise<void> {
    try {
      const stats = await runWithAbortableTimeout(
        (_signal) => fs.stat(this.cachePath),
        FEATURE_FLAGS_IO_TIMEOUT_MS,
        `Stat feature flag cache ${this.cachePath}`,
      );
      if (!stats.isFile()) {
      ...
      const data = await runWithAbortableTimeout(
        (signal) => fs.readFile(this.cachePath, { encoding: 'utf8', signal }),
        FEATURE_FLAGS_IO_TIMEOUT_MS,
        `Read feature flag cache ${this.cachePath}`,
      );
      ...
      if (error?.code !== 'ENOENT') logger.warning('Failed to load feature flags from cache:', error);
      else logger.debug('No feature flag cache found');
      this.loadedFromCache = false;
    }
  }
  ...
  private async fetchFlags(): Promise<void> {
    try {
      // Don't log here - runs async and can interfere with MCP clients. The
      // shared timeout helper combines a hard JS deadline with AbortSignal so
      // both connection and response-body stalls are bounded.
      const config = await runWithAbortableTimeout(async (signal) => {
      ...
      }, FEATURE_FLAGS_FETCH_TIMEOUT_MS, 'Fetch feature flags');
  ...
  private async saveToCache(config: FeatureFlags): Promise<void> {
    try {
      ...
      await runWithAbortableTimeout(
        ...
        FEATURE_FLAGS_IO_TIMEOUT_MS,
        `Create feature flag cache directory ${configDir}`,
      );
      await runWithAbortableTimeout(
        (signal) => fs.writeFile(this.cachePath, JSON.stringify(config, null, 2), { encoding: 'utf8', signal }),
        FEATURE_FLAGS_IO_TIMEOUT_MS,
        `Write feature flag cache ${this.cachePath}`,
      );
