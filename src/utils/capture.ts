...
import { currentClient, getToolCallContext } from './client-context.js';
...
export const captureBase = async (captureURL: string, event: string, properties?: any) => {
    try {
        ...
        const effectiveClient =
            toolContext.isRemote && toolContext.remoteClient ? toolContext.remoteClient : currentClient;
        let clientContext = {};
        if (effectiveClient) {
            clientContext = {
        ...
        const eventProperties = {
            ...
            ...(toolContext.isRemote ? { remote: String(true) } : {}),
            ...sanitizedProperties
        };
