...
async function readCmakeTarget(
  ...
  const sources = Array.isArray(target.sources)
    ? target.sources.map((item) => objectValue(item)?.path).filter((item): item is string => typeof item === 'string').slice(0, 2000).map(slash)
    : [];
...
async function readCmakeMetadataOnce(
  ...
  const codemodelRef = objects.find((item) => {
    const version = objectValue(item.version);
    return item.kind === 'codemodel' && version?.major === 2;
  });
  ...
  if (replyIndex.kind === 'error') {
    return { ...base, codemodel: null, configurations: [], targets: [] };
  }
  if (!codemodelRef) return { ...base, codemodel: null, configurations: [], targets: [] };
