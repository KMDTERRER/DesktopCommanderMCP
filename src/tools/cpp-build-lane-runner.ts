...
import { ensureCmakeFileApiQuery } from './cmake-file-api-query.js';
import { callCppBuildChangeClassification } from './cpp-build-impact-accelerator.js';
import { collectCmakePresetDependencies, type PresetFingerprint } from './cmake-preset-dependencies.js';
import type { BuildAccessPlan } from './cpp-build-access-planner.js';
import type { ResourceLeaseRequest } from '../utils/resource-lease-owner.js';
...
type ConfigureMode = 'never' | 'if_missing' | 'if_needed';
...
export type CppBuildExecuteDependencies = {
  ...
  planBuildAccess: (args: JsonRecord, timeoutMs: number) => Promise<BuildAccessPlan>;
  acquireBuildAccessLease: (
...
function parseArgs(args: JsonRecord): ParsedArgs {
  ...
  if (configureMode !== 'never' && configureMode !== 'if_missing' && configureMode !== 'if_needed') {
    throw new Error("cpp_build_execute.configureMode must be 'never', 'if_missing', or 'if_needed'.");
  }
  ...
  if (configureMode !== 'never' && !configurePreset) {
    throw new Error(`cpp_build_execute.configurePreset is required when configureMode=${configureMode}.`);
  }
  if (configureMode === 'never' && configurePreset) {
    throw new Error('cpp_build_execute.configurePreset requires configureMode=if_missing or if_needed.');
  }
...
function compilerHint(context: JsonRecord): string | null {
  ...
  const toolchains = Array.isArray(cmake?.toolchains) ? cmake!.toolchains as unknown[] : [];
  for (const raw of toolchains) {
    const toolchain = recordValue(raw);
    const compiler = recordValue(toolchain?.compiler);
    if (typeof compiler?.id === 'string') return compiler.id;
  }
...
function stageContextArgs(parsed: ParsedArgs, stage: 'build' | 'test'): JsonRecord {
  const args: JsonRecord = {
    ...
    buildDir: requireBuildDir(parsed),
    configuration: parsed.configuration,
    changedFiles: parsed.changedFiles.length > 0 ? parsed.changedFiles : undefined,
  ...
  if (stage === 'build') {
    ...
    args.useAffectedTargets = parsed.targets.length === 0 && parsed.useAffectedTargets;
  } else {
    args.preset = parsed.testPreset;
    args.tests = parsed.tests.length > 0 ? parsed.tests : undefined;
    args.useAffectedTests = parsed.tests.length === 0 && parsed.useAffectedTests;
...
function stageImpact(context: JsonRecord | null) {
  ...
  const unsupportedChangedFiles = Array.isArray(impact?.unsupportedChangedFiles)
    ? impact!.unsupportedChangedFiles.filter((item): item is string => typeof item === 'string') : [];
  const requiresConfigure = impact?.requiresConfigure === true;
  const reasons = unsupportedChangedFiles.map((file) => `unsupported-impact-file:${file}`);
  if (requiresConfigure) reasons.push('requires-configure');
  ...
  const conservative = unsupportedChangedFiles.length > 0 || requiresConfigure
    || selection === 'snapshot-changed-default' || impact?.selectionComplete === false;
  return {
    mode, conservative, selection, requiresConfigure, unsupportedChangedFiles,
...
async function executeConfigurePreset(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException)?.code === 'ENOENT') {
      throw new Error('CPP_CONFIGURE_PRESET_REQUIRED: project-owned CMakePresets.json is required for preset-driven configure.');
    }
    throw error;
  }
  ...
  if (parsed.buildDir) {
    await ensureCmakeFileApiQuery(root, requireBuildDir(parsed), deadlineAt);
  }
  const cmakeExecutable = await resolveHostCmakeExecutable(root, deadlineAt);
  const configureArgs = ['--preset', parsed.configurePreset!];
  if (parsed.buildDir) configureArgs.push('-B', requireBuildDir(parsed));
...
function mergeConfigurePasses(
  first: ConfigureExecutionResult, second: ConfigureExecutionResult,
): ConfigureExecutionResult {
  if (typeof first.stage.durationMs === 'number' && typeof second.stage.durationMs === 'number') {
    second.stage.durationMs += first.stage.durationMs;
  }
  second.diagnostics = [...first.diagnostics, ...second.diagnostics];
  second.outputTail = boundedTail([first.outputTail, second.outputTail].filter(Boolean).join('\n---\n'));
  return second;
}
...
async function resolveConfiguredBuildDir(
  ...
  // A preset-owned binaryDir is unknown until CMake has evaluated the preset.
  // The first configure therefore cannot contain our client File API query.
  // Re-run the same preset against CMake's discovered tree after the query is
  // installed; CMake remains the owner of preset inheritance/macros/toolchain
  // semantics while the first build receives complete File API replies.
  const fileApiGeneration = await executeConfigurePreset(parsed, deps, root, deadlineAt);
  return mergeConfigurePasses(result, fileApiGeneration);
}
...
type StableBuildOwnership = {
  context: JsonRecord;
  access: BuildAccessPlan;
  releaseBuildLock: () => Promise<void>;
  releaseAccessLease: () => Promise<void>;
};

function buildAccessArgs(parsed: ParsedArgs, context: JsonRecord): JsonRecord {
  const targets = selectedNames(context, 'targets');
  return Object.fromEntries(Object.entries({
    root: parsed.root,
    buildDir: requireBuildDir(parsed),
    configuration: parsed.configuration,
    preset: parsed.buildPreset,
    targets: targets.length > 0 ? targets : undefined,
  }).filter(([, value]) => value !== undefined));
}

async function acquireStableBuildOwnership(
  parsed: ParsedArgs, deps: CppBuildExecuteDependencies, deadlineAt: number,
): Promise<StableBuildOwnership> {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const beforeContext = await callCppBuildContextAcceleratorTool(
      stageContextArgs(parsed, 'build'),
      Math.min(45_000, remaining(deadlineAt, 'build access context preflight')),
    ) as JsonRecord;
    const beforeAccess = await deps.planBuildAccess(
      buildAccessArgs(parsed, beforeContext),
      Math.min(45_000, remaining(deadlineAt, 'build access preflight')),
    );
    const accessLease = await deps.acquireBuildAccessLease(beforeAccess.leaseRequest, deadlineAt);
    ...
    try {
      ...
      const lockedContext = await callCppBuildContextAcceleratorTool(
        stageContextArgs(parsed, 'build'),
        Math.min(45_000, remaining(deadlineAt, 'build access locked context')),
      ) as JsonRecord;
      const lockedAccess = await deps.planBuildAccess(
        buildAccessArgs(parsed, lockedContext),
        Math.min(45_000, remaining(deadlineAt, 'build access locked revalidation')),
      );
      if (beforeAccess.accessFingerprint === lockedAccess.accessFingerprint) {
        return {
          context: lockedContext, access: lockedAccess, releaseBuildLock,
          releaseAccessLease: accessLease.release,
        };
      }
    } catch (error) {
      if (releaseBuildLock) await releaseBuildLock().catch(() => undefined);
      await accessLease.release().catch(() => undefined);
      throw error;
    }
    if (releaseBuildLock) await releaseBuildLock();
    await accessLease.release();
  }
  throw new Error('CPP_BUILD_ACCESS_CHANGED: build access plan changed repeatedly while acquiring build ownership.');
}

function classificationRequiresConfigure(value: JsonRecord): boolean {
  return value.requiresConfigure === true;
}

function configureInvalidators(value: JsonRecord): string[] {
  return Array.isArray(value.configureInvalidatedBy)
    ? value.configureInvalidatedBy.filter((item): item is string => typeof item === 'string')
    : [];
}

function changeClassificationArgs(parsed: ParsedArgs): JsonRecord {
  return Object.fromEntries(Object.entries({
    root: parsed.root, buildDir: requireBuildDir(parsed), changedFiles: parsed.changedFiles,
    configuration: parsed.configuration,
  }).filter(([, item]) => item !== undefined));
}

async function maybeReconfigureInvalidatedBuild(
  parsed: ParsedArgs, deps: CppBuildExecuteDependencies, root: string, deadlineAt: number, alreadyConfigured: boolean,
): Promise<ConfigureExecutionResult | null> {
  if (alreadyConfigured || parsed.changedFiles.length === 0) return null;
  const preflight = await callCppBuildChangeClassification(
    changeClassificationArgs(parsed),
    Math.min(10_000, remaining(deadlineAt, 'reconfigure classification preflight')),
  ) as JsonRecord;
  if (!classificationRequiresConfigure(preflight)) return null;

  const invalidators = configureInvalidators(preflight);
  if (parsed.operation === 'test') {
    throw new Error(
      `CPP_REBUILD_REQUIRED: test-only execution cannot safely absorb build-system changes${invalidators.length ? ` (${invalidators.join(', ')})` : ''}. ` +
      'Run build or build_and_test; use configureMode=if_needed with configurePreset when reconfiguration is allowed.',
    );
  }
  if (parsed.configureMode !== 'if_needed') {
    throw new Error(
      `CPP_RECONFIGURE_REQUIRED: build-system input changed${invalidators.length ? ` (${invalidators.join(', ')})` : ''}. ` +
      'Retry with configureMode=if_needed and an explicit project-owned configurePreset.',
    );
  }

  const buildDir = requireBuildDir(parsed);
  const accessLease = await deps.acquireBuildAccessLease({
    kind: 'build', label: `CMake reconfigure ${path.basename(buildDir)}`, workspaceRoot: root,
    readRoots: [root], writeRoots: [buildDir], coverage: 'conservative',
  }, deadlineAt);
  let releaseBuildLock: (() => Promise<void>) | undefined;
  try {
    releaseBuildLock = await deps.acquireMutationLocks([buildDir], deadlineAt);
    const locked = await callCppBuildChangeClassification(
      changeClassificationArgs(parsed),
      Math.min(10_000, remaining(deadlineAt, 'reconfigure classification locked revalidation')),
    ) as JsonRecord;
    if (!classificationRequiresConfigure(locked)) return null;

    const result = await executeConfigurePreset(parsed, deps, root, deadlineAt);
    if (result.stage.succeeded && !(await configuredTreeExists(root, buildDir, deadlineAt))) {
      return failConfigureResolution(
        result, `Reconfigure preset completed but CMakeCache.txt is unavailable in buildDir: ${buildDir}`,
      );
    }
    return result;
  } finally {
...
export async function runCppBuildLane(
  rawArgs: JsonRecord,
  executionTimeoutMs: number,
  deps: CppBuildExecuteDependencies,
) {
  const parsed = parseArgs(rawArgs);
  const deadlineAt = Date.now() + Math.min(parsed.timeoutMs, executionTimeoutMs);
  ...
  let buildAccess: BuildAccessPlan | null = null;
  let currentStage: StageName = parsed.configureMode !== 'never' ? 'configure'
  ...
  let releaseBuildAccessLease: (() => Promise<void>) | null = null;
  try {
    ...
    // Configure/tree discovery is complete. Do not hold the project configure gate while
    // waiting for source-level build access; independent compiler lanes must remain parallel.
    if (releaseConfigureGate) {
      await releaseConfigureGate();
      releaseConfigureGate = null;
    }

    const configuredDuringResolution = configure.executed && configure.succeeded;
    currentStage = 'configure';
    const invalidatedConfigure = await maybeReconfigureInvalidatedBuild(
      parsed, deps, canonicalRoot, deadlineAt, configuredDuringResolution,
    );
    if (invalidatedConfigure) {
      configure = { ...configure, ...invalidatedConfigure.stage };
      ensureFailureDiagnostic('configure', invalidatedConfigure);
      diagnostics.push(...invalidatedConfigure.diagnostics);
      if (invalidatedConfigure.outputTail) outputTails.push(invalidatedConfigure.outputTail);
      if (!configure.succeeded) {
        return buildExecuteResult(parsed, configure, build, tests, diagnostics, outputTails, null, null);
      }
    }
    currentStage = parsed.operation === 'test' ? 'test' : 'build';

    if (parsed.operation === 'build' || parsed.operation === 'build_and_test') {
      const ownership = await acquireStableBuildOwnership(parsed, deps, deadlineAt);
      buildContext = ownership.context;
      buildAccess = ownership.access;
      releaseBuildLock = ownership.releaseBuildLock;
      releaseBuildAccessLease = ownership.releaseAccessLease;
    } else {
      releaseBuildLock = await deps.acquireMutationLocks([requireBuildDir(parsed)], deadlineAt);
    }
    ...
    if (parsed.operation === 'build' || parsed.operation === 'build_and_test') {
      currentStage = 'build';
      if (!buildContext) throw new Error('CPP_BUILD_ACCESS_CONTEXT_MISSING: stable build ownership returned no build context.');
      ...
      if (!build.succeeded) {
        return buildExecuteResult(parsed, configure, build, tests, diagnostics, outputTails, buildContext, null, buildAccess);
      }
      if (parsed.operation === 'build_and_test' && releaseBuildAccessLease) {
        await releaseBuildAccessLease();
        releaseBuildAccessLease = null;
      }
    }
    ...
    return buildExecuteResult(parsed, configure, build, tests, diagnostics, outputTails, buildContext, testContext, buildAccess);
  } catch (error) {
    ...
    if (releaseBuildLock) await releaseBuildLock();
    if (releaseBuildAccessLease) await releaseBuildAccessLease();
    if (releasePresetLocks) await releasePresetLocks();
  }
}

export class CppBuildLaneRunner {
  constructor(private readonly deps: CppBuildExecuteDependencies) {}

  run(rawArgs: JsonRecord, executionTimeoutMs: number) {
    return runCppBuildLane(rawArgs, executionTimeoutMs, this.deps);
  }
}
...
function buildExecuteResult(
  ...
  buildAccess: BuildAccessPlan | null = null,
) {
  ...
  return {
    ...
    impact: aggregateImpact(stageImpact(buildContext), stageImpact(testContext)),
    buildAccess: buildAccess ? {
      coverage: buildAccess.coverage, fingerprint: buildAccess.accessFingerprint,
      readPathCount: buildAccess.readPaths.length, readRootCount: buildAccess.readRoots.length,
      writePathCount: buildAccess.writePaths.length, writeRoots: buildAccess.writeRoots,
      watchRootCount: buildAccess.watchRoots.length,
      incompleteness: buildAccess.incompleteness, warnings: buildAccess.warnings,
    } : null,
    outputTail,
...
export const CPP_BUILD_EXECUTE_ACCELERATOR_TOOL = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      executionMode: {
        type: 'string', enum: ['auto', 'inline', 'resumable'], default: 'auto',
        description: 'auto keeps fast runs inline but returns buildRunId instead of killing the build when the MCP observation budget is exhausted. inline preserves request-owned timeout semantics. resumable returns an owner-managed buildRunId immediately.',
      },
      changedFiles: {
      ...
      configurePreset: {
        type: 'string',
        description: 'Explicit configure preset, required for configureMode=if_missing or if_needed. It must come from project-owned CMakePresets.json.',
      },
      configureMode: {
        type: 'string', enum: ['never', 'if_missing', 'if_needed'], default: 'never',
        description: 'never is fail-closed. if_missing configures only a missing tree. if_needed also reconfigures an existing tree when authoritative impact marks build-system inputs changed. Both require explicit project-owned configurePreset and never guess compiler/toolchain semantics.',
      },
      useAffectedTargets: {
