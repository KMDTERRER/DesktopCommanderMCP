...
import { MAX_PDF_INPUT_BYTES, MAX_PDF_OUTPUT_BYTES, resourceLimitError } from '../../utils/read-resource-limits.js';
...
async function loadPdfDocumentFromBuffer(filePathOrBuffer: string | Buffer | Uint8Array): Promise<PDFDocumentType> {
    const buffer = typeof filePathOrBuffer === 'string'
        ? await readFileBounded(
            filePathOrBuffer, MAX_PDF_INPUT_BYTES, new AbortController().signal, 'PDF mutation input'
        )
        : filePathOrBuffer;
    if (buffer.byteLength > MAX_PDF_INPUT_BYTES) {
        throw resourceLimitError('PDF mutation input', MAX_PDF_INPUT_BYTES, buffer.byteLength);
    }
...
export async function editPdf(
    ...
    if (output.byteLength > MAX_PDF_OUTPUT_BYTES) {
        throw resourceLimitError('PDF mutation output', MAX_PDF_OUTPUT_BYTES, output.byteLength);
    }
    return output;
}