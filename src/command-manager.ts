...
class CommandManager {
    ...
    async validateExecutable(executable: string, args: string[] = []): Promise<boolean> {
        try {
            const config = await configManager.getConfig();
            const blockedCommands = config.blockedCommands || [];
            const baseName = path.basename(executable).toLowerCase().trim();
            const withoutWindowsExecutableSuffix = baseName.replace(/\.(exe|com|cmd|bat)$/i, '');
            if (blockedCommands.includes(baseName) || blockedCommands.includes(withoutWindowsExecutableSuffix)) {
                return false;
            }

            // Structured argv avoids shell quoting, but an explicit shell executable can
            // still contain a nested command. Reuse the existing command policy for
            // those payloads so direct mode cannot bypass blockedCommands.
            const shellCommandFlags: Record<string, string[]> = {
                cmd: ['/c', '/k'],
                powershell: ['-command', '-c'],
                pwsh: ['-command', '-c', '-commandwithargs', '-cwa'],
                bash: ['-c'],
                sh: ['-c'],
                zsh: ['-c'],
                fish: ['-c'],
            };
            const shellFlags = shellCommandFlags[withoutWindowsExecutableSuffix];
            if (shellFlags) {
                const normalizedArgs = args.map((value) => value.toLowerCase());
                const encodedPowerShellFlags = new Set([
                    '-encodedcommand', '-enc', '-e', '-ec', '-encodedarguments',
                ]);
                if ((withoutWindowsExecutableSuffix === 'powershell' || withoutWindowsExecutableSuffix === 'pwsh')
                    && normalizedArgs.some((value) => encodedPowerShellFlags.has(value))) return false;
                const commandIndex = normalizedArgs.findIndex((value) => shellFlags.includes(value));
                if (commandIndex >= 0) {
                    const nestedCommand = args.slice(commandIndex + 1).join(' ').trim();
                    return nestedCommand.length > 0 && await this.validateCommand(nestedCommand);
                }
            }
            return true;
        } catch (error) {
