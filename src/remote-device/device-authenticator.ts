...
import { isTransientHttpStatus } from './transient-remote-error.js';
...
export class DeviceAuthenticator {
    ...
    private async requestDeviceCode(codeChallenge: string, deviceId?: string): Promise<DeviceAuthResponse> {
        ...
        const { response, data } = await runWithAbortableTimeout(
            async (signal) => {
        ...
        if (!response.ok) {
            const errorMessage = data.error_description || 'Failed to start device flow';
    ...
    private async pollForAuthorization(deviceAuth: DeviceAuthResponse, codeVerifier: string): Promise<AuthSession> {
        ...
        while (attempt < maxAttempts) {
            ...
            try {
                ({ response, data } = await runWithAbortableTimeout(
                    async (signal) => {
                ...
                // Only transport failures reach this catch. Application/protocol
                // errors below are terminal and must never be relabelled as network.
            ...
            // 429/5xx are service availability failures rather than a denial of
            // this device grant. Retry them inside the device-code expiry window.
            if (isTransientHttpStatus(response.status) && attempt < maxAttempts) continue;
