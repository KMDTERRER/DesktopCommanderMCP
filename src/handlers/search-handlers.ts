  ...
  try {
    ...
    if (results.isComplete) {
      ...
      if (results.limitReached) {
        output += `\n⚠️  Search stopped at the configured global maxResults limit.`;
      }
      if (results.wasIncomplete) {
        output += `\n⚠️  Warning: The search ended before every candidate could be processed (for example, access or resource limits).`;
      }
    }
