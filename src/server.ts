...
// Product/onboarding copy must never silently mutate deterministic tool output.
// Keep legacy behavior available only by explicit operator opt-in.
const ENABLE_MODEL_PROMPT_INJECTION = process.env.DESKTOP_COMMANDER_ENABLE_MODEL_PROMPTS === 'true';
...
import { EXTERNAL_MCP_ROUTING_GUIDANCE, listExternalMcpTools, callExternalMcpTool, readExternalMcpCompatUri, readMultipleFilesCompatAware, callExternalMcpCompatUri } from './tools/external-mcp.js';
...
import {
    currentClient, setCurrentClient, runInToolCallContext, getToolCallContext,
    type ToolCallCancellationCleanup,
...
export const server = new Server(
    ...
    },
    {
        ...
        instructions: EXTERNAL_MCP_ROUTING_GUIDANCE,
    },
);
...
function isMcpPassthroughCall(name: string, args: Record<string, unknown>): boolean {
    if (name === 'mcp_call_tool' || name === 'mcp_list_tools') return true;
    if ((name === 'read_file' || name === 'write_file') && typeof args.path === 'string') {
        return isMcpCompatUri(args.path);
    }
    if (name === 'read_multiple_files' && Array.isArray(args.paths)) {
        return args.paths.some((value) => isMcpCompatUri(value));
    }
    return false;
}
...
// Keep MCP lifecycle/negotiation owned by the SDK. index.ts is the single
// owner of server.oninitialized and composes this client-specific phase with
// transport/logging startup work instead of overwriting another callback.
export async function handleServerInitialized(): Promise<void> {
    try {
...
function shouldIncludeTool(toolName: string): boolean {
    // The SDK owns initialization and stores clientInfo before any normal request.
    // Read that canonical value directly so tools/list cannot race post-init side effects.
    const clientName = server.getClientVersion()?.name ?? currentClient?.name;

    // Exclude these tools for desktop-commander client (DC-specific meta-tools not useful when DC itself is the client)
    if (clientName === 'desktop-commander-app') {
    ...
    // Example: if (toolName === 'some_tool' && clientName === 'some_client') return false;
        ...
        const allTools = [
            ...
            // Filesystem tools
            {
                ...
                description: `
                        ...
                        - Excel (.xlsx): Returns JSON 2D array
            ...
            },
            {
                ...
                description: `
                        ...
                        - Excel (.xlsx): JSON 2D array or {"SheetName": [[...]]}
            ...
            // Editing tools
            {
                ...
                description: `
                        ...
                        EXCEL FILES (.xlsx) - Range Update mode:
                        Takes:
            ...
            // Terminal tools
            {
                ...
                description: `
                        ...
                        STRUCTURED EXECUTION (PREFERRED WHEN NO SHELL SYNTAX IS NEEDED):
                        - For builds, tests, linters, and one-shot programs, prefer executable + args + cwd + env.
                          This launches the program directly (shell=false), avoiding shell quoting, cd wrappers, and env preambles.
                        - Use command + shell only when pipes, redirection, shell built-ins, compound operators, or shell expansion are required.
                        - Set execution_kind=finite for one-shot work; prompt heuristics are disabled so output ending in >/$/# cannot cause a false REPL handoff.
                        ...
                        WAIT BOUNDARY:
                        - timeout_ms is capped at 45000ms so starting a quiet process cannot pin one RPC indefinitely
                        - For longer finite work, start with a short timeout and observe it with desktop-accelerators/wait_process

                        SMART DETECTION:
                        - Detects REPL prompts (>>>, >, $, etc.) for auto/interactive execution; finite/service execution skips prompt heuristics
            ...
            },
            {
                ...
                description: `
                        ...
                        - 'offset' (start line, default: 0)
                          ...
                          * With reader_id + offset=0: maintain an independent cursor so concurrent consumers do not consume each other's output
                        ...
                        Process finished execution
                        Stall warning (still running; no output for stall_timeout_ms; process is not terminated)
            ...
            },
            {
                ...
                description: `
                        ...
                        WAIT BOUNDARY:
                        - timeout_ms is capped at 45000ms; use repeated bounded calls instead of one multi-minute RPC
                        - Interactions on the same PID are serialized across the full snapshot/input/response cycle so concurrent callers cannot mix logical replies
            ...
            {
                name: "mcp_list_tools",
                description: `Preferred discovery entry point for task-specific external MCP capabilities. For codebase navigation, references, implementations, diagnostics, or refactoring, use this before generic text search/read when a configured semantic server may answer the task. Tool schemas are returned only when a specific tool is requested. ${CMD_PREFIX_DESCRIPTION}`,
                inputSchema: zodToJsonSchema(McpListToolsArgsSchema),
                annotations: {
            ...
            {
                name: "mcp_call_tool",
                description: `Execute an exact tool discovered on a configured external MCP server. For code intelligence, prefer applicable semantic tools over grep/search/full-file reads. Inspect the exact downstream schema first with mcp_list_tools(server, tool). This proxy may invoke either read-only or mutating downstream tools, so downstream semantics govern safety. ${CMD_PREFIX_DESCRIPTION}`,
                inputSchema: zodToJsonSchema(McpCallToolArgsSchema),
                annotations: {
...
server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest, extra): Promise<ServerResult> => {
    ...
    const isRemote = !!(metadata && typeof metadata === 'object' && metadata.remote);
    const remoteClient = isRemote
        ? (metadata.clientInfo && (metadata.clientInfo.name || metadata.clientInfo.version)
            ? metadata.clientInfo
            : { name: 'remote-unknown', version: 'unknown' })
        : null;
    const cancellationCleanups = new Set<ToolCallCancellationCleanup>();
    const cancellationState: CancellationState = {};
    // SDK-backed calls provide a request AbortSignal, but direct/legacy harnesses
    // and some embedded callers may omit it. Cancellation is optional metadata,
    // not a precondition for executing otherwise valid tools.
    const requestSignal = extra?.signal ?? new AbortController().signal;
    const cancelOwnedWork = () => {
        const cause = classifyRequestAbortReason(requestSignal.reason, 'client_cancelled');
        setCancellationState(
            cancellationState, cause,
            requestSignal.reason instanceof Error ? requestSignal.reason.message : undefined,
        );
        const cleanups = [...cancellationCleanups];
        cancellationCleanups.clear();
        for (const cleanup of cleanups) void Promise.resolve(cleanup(cause)).catch(() => {});
    };
    if (requestSignal.aborted) cancelOwnedWork();
    else requestSignal.addEventListener('abort', cancelOwnedWork, { once: true });

    const requestMetadata = metadata && typeof metadata === 'object' && !Array.isArray(metadata)
        ? metadata as Record<string, unknown>
        : undefined;
    const runCall = () => runInToolCallContext(
        { isRemote, remoteClient, requestMetadata, requestSignal, cancellationState, cancellationCleanups },
...
async function handleCallToolRequest(request: CallToolRequest): Promise<ServerResult> {
    ...
    try {
        ...
        if (toolContext.isRemote) {
            telemetryData.remote = String(metadata?.remote ?? true);
            const remoteClient = toolContext.remoteClient ?? { name: 'remote-unknown', version: 'unknown' };
        ...
        switch (name) {
            ...
            case "mcp_list_tools":
                try {
                    result = await listExternalMcpTools(args as Parameters<typeof listExternalMcpTools>[0]);
                } catch (error) {
            ...
            case "mcp_call_tool":
                try {
                    result = await callExternalMcpTool(args as Parameters<typeof callExternalMcpTool>[0]);
                } catch (error) {
                    const message = error instanceof Error ? error.message : String(error);
            ...
            case "read_file": {
                ...
                if (isMcpCompatUri(parsed.path)) {
                    const unsupportedOptions = unsupportedMcpReadFileOptions(parsed);
                    if (unsupportedOptions.length > 0) {
                        throw new Error(`mcp:// read_file does not support filesystem read options: ${unsupportedOptions.join(', ')}.`);
                    }
                    result = await readExternalMcpCompatUri(parsed.path, parsed.options);
                } else {
            ...
            case "read_multiple_files": {
                ...
                if (!parsed.paths.some((value) => isMcpCompatUri(value))) {
                    result = await handlers.handleReadMultipleFiles(args);
                    break;
                }
                result = await readMultipleFilesCompatAware(
                    parsed.paths,
            ...
            case "write_file": {
                ...
                if (isMcpCompatUri(parsed.path)) {
                    ...
                    result = await callExternalMcpCompatUri(parsed.path, parsed.content);
                } else {
        ...
        try {
            ...
            if (!isMcpPassthrough && argSchema && result && Array.isArray((result as any).content)) {
