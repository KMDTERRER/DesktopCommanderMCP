...
const { createMcpbPackageJson } = require('./mcpb-package-json.cjs');
...
// Step 6: Create package.json with the same production resolution policy as the npm package.
// Keep runtime dependencies, optional dependencies, overrides, and the Node engine contract in sync.
const bundlePackageJson = createMcpbPackageJson(packageJson, manifest);
