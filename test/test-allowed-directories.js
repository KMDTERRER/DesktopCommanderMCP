...
const TEST_ROOT_PATH = isWindows ? path.parse(HOME_DIR).root : '/';
...
async function testHomeAllowedDirectory() {
    ...
    // The repository test directory is accessible only when it is actually inside HOME.
    const testDirRelative = path.relative(HOME_DIR, TEST_DIR);
    const isTestDirInHome = testDirRelative === '' ||
        (!testDirRelative.startsWith('..') && !path.isAbsolute(testDirRelative));
    assert.strictEqual(testDirAccess, isTestDirInHome, 'Test directory access should follow HOME containment');
    assert.strictEqual(testFileAccess, isTestDirInHome, 'Test file access should follow HOME containment');
...
if (import.meta.filename === process.argv[1]) {
  runTests().then(success => {
    process.exitCode = success === false ? 1 : 0;
  }).catch(error => {
