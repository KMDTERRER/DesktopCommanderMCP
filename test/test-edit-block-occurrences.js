...
async function testExactNumberOfOccurrences() {
  ...
  try {
    ...
    assert.strictEqual(result.content[0].type, 'text', 'Result should be text');
    assert.ok(
      result.content[0].text.includes('This line has been replaced correctly.'),
      'Success response should preview the replacement'
    );
...
This is a MODIFIED target line in the header.`,
    ...
    assert.strictEqual(result.content[0].type, 'text', 'Result should be text');
    assert.ok(
      result.content[0].text.includes('MODIFIED target line in the header'),
      'Success response should preview the header edit'
    );
...
This is a MODIFIED target line in the footer.`,
    ...
    assert.strictEqual(result.content[0].type, 'text', 'Result should be text');
    assert.ok(
      result.content[0].text.includes('MODIFIED target line in the footer'),
      'Success response should preview the footer edit'
    );
