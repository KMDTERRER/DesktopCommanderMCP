 ...
 * this with a scaled-down fetch-like promise that ignores abort. The timing
 * ratio is what matters here; real network timeouts are covered above.
 */
async function testBrokenAbortControllerSimulation() {
  // Simulate a fetch that ignores AbortController (the Windows bug)
  const simulatedOsTimeoutMs = 2000;
  const simulatedHardTimeoutMs = 500;
  function brokenFetch() {
    return new Promise((resolve, reject) => {
      setTimeout(() => reject(new Error('Simulated OS TCP timeout')), simulatedOsTimeoutMs);
    });
  }

  // Scale the synthetic delay down while preserving the broken-abort behavior.
  const start1 = Date.now();
  const controller = new AbortController();
  setTimeout(() => controller.abort(), simulatedHardTimeoutMs);
  try {
    await brokenFetch(); // ignores the abort signal
  } catch (err) {
    // Expected after the scaled OS timeout.
  }
  ...
  const timeoutPromise = new Promise((_, reject) =>
    setTimeout(() => reject(new Error('Hard timeout')), simulatedHardTimeoutMs)
  );
  try {
    await Promise.race([brokenFetch(), timeoutPromise]);
  } catch (err) {
    // Expected from the hard timeout.
  }
  const elapsed2 = Date.now() - start2;
  console.log(`   Broken AbortController (Promise.race fix): ${elapsed2}ms`);

  assert(
    elapsed1 >= simulatedOsTimeoutMs * 0.8,
    `Broken fetch resolved too early: ${elapsed1}ms`
  );
  assert(
    elapsed2 < simulatedHardTimeoutMs * 2,
    `Promise.race hard timeout took too long: ${elapsed2}ms`
  );
...
async function main() {
  ...
  const results = await Promise.all([
    runTest('AbortController interrupts fetch to black-hole server (2s)', testAbortControllerBehavior),
    runTest('Current fetchFlags pattern respects 5s timeout (black-hole)', testCurrentPatternBlackHole),
    runTest('AbortController-only pattern (pre-fix baseline) against black-hole', testAbortControllerOnlyBlackHole),
    runTest('Current pattern aborts before 15s slow response arrives', testCurrentPatternSlowResponse),
    runTest('New-user onboarding path does not block >8s on slow network', testNewUserOnboardingPath),
    runTest('Promise.race fixes broken AbortController (simulated #465)', testBrokenAbortControllerSimulation),
  ]);
