...
async function isShellAvailable(shellPath) {
  try {
    if (os.platform() === 'win32' && shellPath.startsWith('/')) return false;
...
async function getShellFromCommand(shellPath = null) {
  try {
    ...
    if (shellPath === 'cmd' || shellPath === 'cmd.exe') {
      ...
      command = 'Write-Output (Get-Process -Id $PID).ProcessName';
    }
    ...
    if (result.content && result.content[0] && result.content[0].text) {
      ...
      for (const line of lines) {
        if (!line.includes('PID') && 
            ...
            line.trim() !== '') {
          const shellName = line.trim();
          return shellPath === 'cmd' || shellPath === 'cmd.exe'
            ? shellName.split(/[\\/]/).pop().toLowerCase()
            : shellName;
        }
      }
    }
