...
function getPythonCommand() {
  ...
  throw new Error(`No usable Python command found in PATH (tried: ${candidates.join(', ')})`);
}

async function testSplitStreamPromptSettling() {
  const script = [
    "process.stdin.setEncoding('utf8');",
    "process.stdout.write('READY\\n>>> ');",
    "process.stdin.once('data', () => {",
    "  process.stderr.write('>>> ');",
    "  setTimeout(() => process.stdout.write('LATE_STDOUT\\n'), 80);",
    "  setTimeout(() => process.stderr.write('>>> '), 90);",
  ...
  assert(Number.isInteger(pid) && pid > 0, `Synthetic REPL failed to start: ${JSON.stringify(started)}`);
  await readProcessOutput({ pid, timeout_ms: 500 });
  const response = await interactWithProcess({ pid, input: 'go', wait_for_prompt: true, timeout_ms: 2000 });
  assert(response.content[0].text.includes('LATE_STDOUT'),
    `Prompt completion lost delayed stdout from a separate pipe: ${response.content[0].text}`);
}
...
async function testEnhancedREPL() {
  ...
  await testSplitStreamPromptSettling();
