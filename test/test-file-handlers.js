...
async function testCanHandle() {
  ...
  // ExcelJS-backed handler owns only XLSX. Legacy BIFF .xls and macro-enabled
  // variants must not be routed into an incompatible XLSX parser.
  assert.ok(excelHandler.canHandle('anything.xlsx'), 'Excel handler should handle .xlsx');
  assert.equal(excelHandler.canHandle('file.xls'), false, 'Excel handler must not claim legacy .xls');
  assert.equal(excelHandler.canHandle('file.xlsm'), false, 'Excel handler must not claim unsupported .xlsm');
