...
Prefer the structured process form whenever the operation does not require shell
syntax. Pass `executable` and `args[]` separately, use absolute `cwd` instead of
writing `cd ... &&`, and use the `env` overlay instead of shell-specific `set` /
`$env:` preambles. This avoids quoting differences between PowerShell, cmd and
Unix shells.

- **Finite / one-shot work** (builds, tests, linters, normal CLIs) → use
  `executable + args[]`, `execution_kind=finite`, `pty=never`, and an absolute
  `cwd` when the command is project-relative. If it is still running after the
  bounded start window, continue with `read_process_output` / the available wait
  primitive instead of rebuilding the command.
- **Long-running services** → use structured execution with
  `execution_kind=service`; do not fake a service by appending shell sleeps.
- **Interactive sessions** (REPLs, SSH, DB shells, installers that prompt) → use
  structured execution with `execution_kind=interactive`, `pty=auto`, then
  `interact_with_process(pid, "...")`. End with `force_terminate` if needed.
- **Use legacy `command` / shell mode only when shell semantics are actually
  required**: pipes, redirection, wildcard expansion, shell built-ins, compound
  operators, or shell activation/state. Keep those command strings short.
- Navigation is inspection: prefer `list_directory` and `get_file_info`; when a
  process needs a directory, set `cwd` directly instead of invoking `cd`.
...
```
start_process(executable="python3", args=["-i"], execution_kind="interactive", pty="auto")
